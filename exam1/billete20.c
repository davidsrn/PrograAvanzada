...
void a_hidrogen(char* program) {
	...
	printf("Billete de 20  %i intentando dar su billete.\n", getpid());
	mutex_wait(semid, MUTEX);
	printf("Billete de 20 %i intentando dar el billete a benji quien ha dado %i\n", getpid(), semctl(semid, BENJI, GETVAL, 0));
  sem_wait(semid, BENJI, 1);

  if (semctl(semid, BILLETE20, GETVAL, 0) == 1) {
    printf("Ya hay un billeted de 20 en la mesa.\n");
		// flag = 0;
    sem_signal(semid, BENJI, 1);
  }
  else{
    sem_signal(semid, BILLETE20, 1);
  	printf("%i dio un billete de 20.\n", getpid());
  }
