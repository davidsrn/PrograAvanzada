...
void a_hidrogen(char* program) {
	...
	printf("Billete de 100  %i intentando dar su billete.\n", getpid());
	mutex_wait(semid, MUTEX);
	printf("Billete de 100 %i intentando dar el billete a benji quien ha dado %i\n", getpid(), semctl(semid, BENJI, GETVAL, 0));
  sem_wait(semid, BENJI, 1);

  if (semctl(semid, BILLETE100, GETVAL, 0) == 1) {
    printf("Ya hay un billeted de 100 en la mesa.\n");
		// flag = 0;
    sem_signal(semid, BENJI, 1);
  }
  else{
    sem_signal(semid, BILLETE100, 1);
  	printf("%i dio un billete de 100.\n", getpid());
  }
